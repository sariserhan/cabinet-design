...
`npm run check` covers 300 semantic tests, 31 backend ownership/concurrency/audit tests, mocked provider contract tests, TypeScript, lint and draft-artifact integrity. `npm run test:e2e` adds 35 browser tests, which CI does not yet run. These are software checks, not measured manufacturer-data accuracy.
