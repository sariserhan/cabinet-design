...
Choose **Render** beside **3D preview** for the WebGL view. It uses the current room outline, openings, cabinet dimensions, assemblies, and finish, with lighting and shadows. Drag to orbit, right-drag to pan, and scroll to zoom (two fingers pan/zoom on touch screens). The wheel zooms towards the pointer rather than the middle of the screen, so what you lean in to look at stays in frame and the next drag orbits around it. For eye-level movement instead of orbiting, tick **Eye-level walkthrough**: drag to look, move with WASD or the arrow keys, and the walk refuses to pass through cabinets, appliances and solid partitions while door openings stay passable. **Reset camera** fits the room; **Cutaway walls** hides walls facing away from the room interior so the layout stays visible. **Download PNG** exports the current camera view at the canvas resolution.
...
On a machine with a GPU the view draws at twice the screen's pixel density and takes eight multisamples; on software rendering it keeps one pixel per pixel and four. The reason is thin metal — a tap, a handle, the edge of an appliance — whose highlight can be narrower than a pixel and jump to the next one as the camera turns. Extra multisamples fix the edges of shapes and do nothing for that; shading each pixel more than once is what does. Measured against the same view once it has settled, it removes about a third of the difference a moving frame shows.
