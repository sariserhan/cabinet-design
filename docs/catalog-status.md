...
- `OPENAI_API_KEY` is present in the local `.env.local`; `ANTHROPIC_API_KEY` is not. Checked on 2026-09-16 with a model listing, which sends no document content: the key is accepted and 124 models are reachable, including the gpt-5, gpt-4.1 and gpt-4o families. The model ID remains explicit operator configuration with no allowlist in code.
- The worker pipeline was exercised end to end on 2026-09-16 against the real deployment. Source download, SHA-256 verification, PyMuPDF page extraction and page-image upload all succeeded, and the draft import stored records for all 32 selected pages. A one-page compile of physical page 22 with `openai` / `gpt-4.1` then failed at the provider call: HTTP 429, "You have no credits remaining." No extraction output was produced and no tokens were billed. Extraction accuracy therefore remains unmeasured, now blocked on provider account credit rather than on credentials or on the pipeline.
- Provider tests use explicitly synthetic mocked responses; no live extraction success is claimed.
- Human-verified benchmark records: 0. The app and integrity checker never relabel automated annotations as human truth.
- Browser PDF upload silently truncates a large source. A 12,408,676-byte PDF, confirmed complete in the page itself (`File.size` and the read `ArrayBuffer` both 12,408,676), was stored as 10,436,608 bytes. Because the SHA-256 is computed from whatever arrives, the truncated document gets a self-consistent but wrong hash and passes every integrity check downstream; it then fails later with a misleading "Invalid selected page numbers" from the page extractor. The server-side loader (`/api/benchmark-source`, which reads the file with `readFile`) stores the same PDF correctly at 12,408,676 bytes with the canonical `18424d5f…` hash. The losing hop, between the Next route handler and the Convex HTTP action, has not been isolated. Until it is, load sources through the pinned-source button rather than the upload dialog.
