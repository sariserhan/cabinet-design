...
Storage preferences, site tasks and client selection boards persist in local drafts, saved designs and JSON exports. Cloud persistence requires deploying the updated shared design schema. Cloud saves read back these fields and pause with the local draft retained if an older backend drops them. At implementation time, remote deployment is awaiting explicit authorization; no public frontend deployment or client communication was performed.
...
The new selection board is included in the shared design schema and cloud readback checks. Deploying the shared schema to the development backend remains pending authorization; older cloud saves pause when new fields are not preserved. Visual milestone history is local to the browser and backed up through its own export.
